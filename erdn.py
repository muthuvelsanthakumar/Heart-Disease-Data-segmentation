...
def calculate_intervals(waves):
    """
    Calculate QT Interval, ST Segment, and RR intervals.
    :param waves: Dictionary with extracted wave parameters (P, QRS, T)
    :return: Dictionary with calculated intervals
    """
    intervals = {}
    
    # Calculate QT Interval (start of Q wave to end of T wave)
    if "QRS_complex" in waves and "T_wave" in waves:
        qrs_start = waves["QRS_complex"][0]["start"]
        t_end = waves["T_wave"][-1]["end"]
        intervals["QT_Interval"] = t_end - qrs_start
    
    # Calculate ST Segment (end of S wave to start of T wave)
    if "QRS_complex" in waves and "T_wave" in waves:
        s_end = waves["QRS_complex"][-1]["end"]
        t_start = waves["T_wave"][0]["start"]
        intervals["ST_Segment"] = t_start - s_end
    
    # Calculate RR Intervals (time between successive R peaks)
    if "QRS_complex" in waves:
        r_peaks = [wave["peak"] for wave in waves["QRS_complex"]]
...
def generate_combined_csv_output(calibrated_leads, num_leads=12):
    ...
    for i in range(num_leads):
        ...
        intervals = calculate_intervals(waves)
        ...
        for interval_name, interval_value in intervals.items():
            if isinstance(interval_value, np.ndarray):
                for value in interval_value:
                    all_leads_data.append({
                        "Lead": lead_name,
                        "Wave_Type": interval_name,
                        "Start": None,
                        "Peak": None,
                        "End": None,
                        "Amplitude": None,
